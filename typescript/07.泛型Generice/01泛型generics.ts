...
// 函数泛型的写法 函数名<变量>(参数: 类型): 函数返回值类型
// T可以是任意的变量名, 表示先不对函数进行类型指定, 由后面的传参来决定
function GenericsFn<T>(arg: T): T {
  return arg
}
const res1 = GenericsFn('llllll') // string

function jiaoHuanWeiZhi<T, U>(tuple: [T, U]): [U, T] {
  return [tuple[1], tuple[0]]
}
const res2 = jiaoHuanWeiZhi(['hello', 9527])
console.log(res2)